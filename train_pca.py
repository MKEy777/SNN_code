...
OUTPUT_DIR_BASE = r"PCA_结果_中文_准确率早停"  
OUTPUT_SIZE = 3 
T_MIN_INPUT = 0.0 
T_MAX_INPUT = 1.0  
TEST_SPLIT_SIZE = 0.2 
...
def run_training_session(current_hyperparams: Dict, fixed_params_dict: Dict, run_id: int) -> float:
    ...
    run_specific_output_dir_name = f"{base_prefix_for_dir}_PCA_验证准确率早停_自定义初始化_{run_timestamp}"
    ...
    files_internal_prefix = f"最终最佳验证准确率_PCA_早停_自定义初始化" 
    ...
    print(f"PCA保留的主成分数量 : {k_features_to_select}")
    ...
    print(f"应用PCA特征降维，保留 {k_features_to_select} 个主成分...")
    if k_features_to_select > X_train_full.shape[1]:
        print(f"警告: 请求的 K_FEATURES ({k_features_to_select}) 大于训练集特征数 ({X_train_full.shape[1]})。将保留所有主成分。")
    ...
    pca = PCA(n_components=k_features_to_select)
    X_train_selected = pca.fit_transform(X_train_full)
    X_val_selected = pca.transform(X_val_full)
    print(f"PCA特征降维完成。训练集降维后特征形状: {X_train_selected.shape}, 验证集降维后特征形状: {X_val_selected.shape}")
    ...
    with open(report_filename, 'w', encoding='utf-8') as f:
        params_to_save_report = {
            **all_params_for_naming_and_saving,
            ...
            'PCA保留的主成分数量': k_features_to_select,
            '特征选择方法': 'PCA'
        }
...
if __name__ == "__main__":
    ...
    if best_hyperparams_combo_overall:
        ...
        summary_file_name = f"网格搜索摘要_PCA_验证准确率早停_自定义初始化_{time.strftime('%Y%m%d_%H%M%S')}.json"
