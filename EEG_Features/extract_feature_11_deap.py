...
INPUT_DIR = r"C:\Users\VECTOR\Desktop\DeepLearning\SNN_code\PerSession_MAT_Deap"
OUTPUT_DIR_FEATURES = r"C:\Users\VECTOR\Desktop\DeepLearning\SNN_code\deap_features"
FS = 128  # Sampling frequency of DEAP dataset
N_FEATURES_PER_COMBINATION = 11
N_BANDS = 4  # Assuming 4 frequency bands
N_CHANNELS = 32  # Number of EEG channels in DEAP
...
def process_single_file(fpath, output_dir_features, fs=FS):
    ...
    seg_X = mat_data['data']  # Assuming preprocessed data key is 'data'
    seg_y = mat_data['labels']  # Assuming labels key is 'labels'

    n_segments_in_file = seg_X.shape[0]
    if n_segments_in_file == 0:
        print(f"  Skipping {fname} as it contains no segments.")
        return
...
def run_deap_processing(input_dir, output_dir_features):
    ...
    print(f"\nProcessing finished for {total_files} files in {elapsed_total:.2f} seconds.")
