...
OUTPUT_DIR_BASE = r"SNN_RFselect_SEEDIV_DynamicTime_2HL_EngPlot"
OUTPUT_SIZE = 4
T_MIN_INPUT = 0.0
T_MAX_INPUT = 1.0
TEST_SPLIT_SIZE = 0.1
...
def build_filename_prefix(params: Dict[str, Any]) -> str:
    ...
    kfeat = params.get('K_FEATURES', 'KNA')
    rf_n = params.get('RF_N_ESTIMATORS', 'rfNNA')
    rf_d = params.get('RF_MAX_DEPTH', 'rfDNA')
    rf_d_str = str(rf_d) if rf_d is not None else "None"
    rf_mf = params.get('RF_MAX_FEATURES', 'rfMFNA')
    if isinstance(rf_mf, float):
        rf_mf_str = str(rf_mf).replace('.', 'p')
    else:
        rf_mf_str = str(rf_mf)
    ...
    return (f"SNN_RFk{kfeat}_rfN{rf_n}_rfD{rf_d_str}_rfMF{rf_mf_str}_lr{lr_str}_bs{bs}"
...
def run_training_session(current_hyperparams: Dict, fixed_params_dict_input: Dict, run_id: int) -> float:
    ...
    _K_FEATURES = current_hyperparams['K_FEATURES']
    _RF_N_ESTIMATORS = current_hyperparams['RF_N_ESTIMATORS']
    _RF_MAX_DEPTH = current_hyperparams['RF_MAX_DEPTH']
    _RF_MIN_SAMPLES_SPLIT = current_hyperparams['RF_MIN_SAMPLES_SPLIT']
    _RF_MIN_SAMPLES_LEAF = current_hyperparams['RF_MIN_SAMPLES_LEAF']
    _RF_MAX_FEATURES = current_hyperparams['RF_MAX_FEATURES']
    ...
    print(f"Data loaded. Original feature shape: {features_data.shape}, Original label shape: {labels_data.shape}")
    ...
    print(f"Split. Train set size: {X_train_full.shape[0]}, Validation set size: {X_val_full.shape[0]}")

    print(f"Starting Random Forest feature selection, selecting k={_K_FEATURES} features...")
    rf_feature_selector = RandomForestClassifier(
        n_estimators=_RF_N_ESTIMATORS,
        max_depth=_RF_MAX_DEPTH,
        min_samples_split=_RF_MIN_SAMPLES_SPLIT,
        min_samples_leaf=_RF_MIN_SAMPLES_LEAF,
        max_features=_RF_MAX_FEATURES,
    ...
    actual_k_features = _K_FEATURES
    if _K_FEATURES > X_train_full.shape[1]:
        print(f"Warning: K_FEATURES ({_K_FEATURES}) is greater than available features ({X_train_full.shape[1]}). Using all available features.")
        actual_k_features = X_train_full.shape[1]
    
    indices = np.argsort(importances)[::-1][:actual_k_features]
    X_train_selected = X_train_full[:, indices]
    X_val_selected = X_val_full[:, indices]
    all_params['K_FEATURES_USED'] = actual_k_features # Store actual K used for saving

    print(f"Feature selection complete. Selected train feature shape: {X_train_selected.shape}, Selected validation feature shape: {X_val_selected.shape}")
    ...
    print(f"Encoded train feature shape: {X_train_encoded.shape}, Encoded validation feature shape: {X_val_encoded.shape}")
    ...
    model.add(SpikingDense(units=_HIDDEN_UNITS_1,
                           input_dim=X_train_encoded.shape[1], # Should be actual_k_features
                           outputLayer=False,
                           name='hidden_layer_1'))
    ...
    for epoch in range(_NUM_EPOCHS):
        ...
        if val_acc > best_val_acc_during_training + _EARLY_STOPPING_MIN_DELTA:
            ...
            patience_counter = 0
            print(f"  Validation accuracy improved to {best_val_acc_during_training:.4f}. Resetting patience.")
        else:
            patience_counter += 1
            print(f"  Validation accuracy did not significantly improve. Patience: {patience_counter}/{_EARLY_STOPPING_PATIENCE}")
        if patience_counter >= _EARLY_STOPPING_PATIENCE:
            print(f"Early stopping triggered at epoch {epoch+1}.")
            actual_stopped_epoch = epoch + 1
            break
    if best_model_state_dict:
        print("Loading best model state (highest validation accuracy)...")
        model.load_state_dict(best_model_state_dict)
    else:
        print("Warning: No better model state saved during training. Using the last model state.")
    ...
    print("\nClassification Report (on validation set with best model):")
    print(report_str)
    ...
    with open(report_filename, 'w', encoding='utf-8') as f:
        ...
        f.write(f"Feature Selection: Random Forest (k_requested={_K_FEATURES}, k_used={all_params.get('K_FEATURES_USED', _K_FEATURES)}, n_est={_RF_N_ESTIMATORS}, depth={_RF_MAX_DEPTH}, max_feat={_RF_MAX_FEATURES})\n")
        f.write(f"Actual Epochs: {actual_stopped_epoch} / {_NUM_EPOCHS}\n")
        f.write(f"Best validation accuracy during training: {best_val_acc_during_training:.4f}\n")
        f.write(f"Final validation accuracy of best model: {final_val_acc:.4f}\n\n")
...
if __name__ == "__main__":
    ...
    print(f"Executing training for {num_combinations} hyperparameter combinations.")
    ...
    for i, params_combo in enumerate(hyperparam_combinations):
        ...
        print(f"Run ID {i+1} completed. Duration: {run_duration_str} ({run_duration_seconds:.2f}s). Final validation accuracy: {validation_accuracy:.4f}")
        ...
        all_results_summary.append({
            ...
            'Duration (s)': round(run_duration_seconds, 2),
            'Duration (H:M:S)': run_duration_str
        })
        ...
        print(f"{'='*30} Finished Combination {i+1}/{num_combinations} {'='*30}")
    ...
    print(f"\n--- All {num_combinations} hyperparameter trials completed ---")
    print(f"Total duration: {total_duration_str} ({total_duration_seconds:.2f}s)")
    if best_hyperparams_combo_overall:
        print(f"Best overall validation accuracy: {best_accuracy_overall:.4f}")
        print(f"Corresponding best hyperparameter combination:\n{json.dumps(best_hyperparams_combo_overall, indent=2, ensure_ascii=False)}")
        summary_file_name = f"grid_search_summary_SNN_RFselect_2HL_EngPlot_{time.strftime('%Y%m%d_%H%M%S')}.json"
        ...
        for k, v_best in best_hyperparams_combo_overall.items():
            if isinstance(v_best, np.integer): serializable_best_hyperparams[k] = int(v_best)
            elif isinstance(v_best, np.floating): serializable_best_hyperparams[k] = float(v_best)
            else: serializable_best_hyperparams[k] = v_best
        serializable_all_results = []
        for res in all_results_summary:
            s_params_combo = {}
            for k_p, v_p in res['Hyperparameter Combination'].items():
                if isinstance(v_p, np.integer): s_params_combo[k_p] = int(v_p)
                elif isinstance(v_p, np.floating): s_params_combo[k_p] = float(v_p)
                else: s_params_combo[k_p] = v_p
            serializable_all_results.append({**res, 'Hyperparameter Combination': s_params_combo})
        summary_data = {
            "Overall Best Validation Accuracy": best_accuracy_overall,
            "Best Hyperparameter Combination": serializable_best_hyperparams,
            ...
            "Total Duration (s)": round(total_duration_seconds, 2)
        }
        with open(summary_file_path, 'w', encoding='utf-8') as f:
            json.dump(summary_data, f, indent=4, ensure_ascii=False)
        print(f"Summary report saved to: {summary_file_path}")
    else:
        print("No successful training runs or all runs resulted in zero/NaN accuracy.")