...
# 处理单个被试数据 
# --------------------------------------------
def process_subject_data(filepath, fs=128, window_sec=4, step_sec=2, only_EEG=True):
    """
    加载、基线校正并分割单个被试数据（无滤波）。
    参数:
        filepath: .dat 文件路径。
        fs: 采样频率。
        window_sec: 窗口长度（秒）。
    ...
    返回:
        tuple: (segmented_data, segmented_labels)
               segmented_data 形状: (n_segments, 32, window_samples)
    ...
    print(f"  基线已校正。准备分割的数据形状: {data_corrected.shape}") # (n_trials, 32, timepoints - baseline_samples)


    # 二分类标签
    labels_quantized_trial_all = labels_quantization(labels_raw, 2)
    print(f"  二分类标签形状: {labels_quantized_trial_all.shape}") # (2, n_trials)

    # 滑动窗口分割
    window_samples = window_sec * fs
    step_samples = step_sec * fs
    # 直接使用 data_corrected 进行分割
    _, num_channels, segmentable_timepoints = data_corrected.shape # 获取维度

    all_segments = []
    all_segment_labels_quantized = []

    for trial_idx in range(num_trials):
        trial_data = data_corrected[trial_idx]  # (32, timepoints)
        trial_label_pair_quantized = labels_quantized_trial_all[:, trial_idx] # (2,)

        start = 0
        while start + window_samples <= segmentable_timepoints:
            # 分割数据，形状 (32, window_samples)
            segment = trial_data[:, start:start + window_samples]
            all_segments.append(segment)
            all_segment_labels_quantized.append(trial_label_pair_quantized)
            start += step_samples

    segmented_data_np = np.array(all_segments)  # (n_segments, 32, 512)
...
if __name__ == '__main__':
    ...
    for filename in filenames:
        ...
        segmented_data, segmented_labels = process_subject_data(
            subject_filepath, fs, window_sec, step_sec, only_EEG
        )

        # 准备保存的数据结构
        mat_data = {
            'data': segmented_data,  # (n_segments, 32, 512)
            'labels': segmented_labels  # (n_segments, 2)
        }

        # 定义输出文件名（包含无滤波信息）
        output_filename = f"{subject_id}_win{window_sec}s_step{step_sec}s_nofilter_binaryVA.mat"
